...
export const getViewByTime = async (req: Request, res: Response) => {
  const time = req.params.time;
  switch (time) {
    case "days":
      const viewDay = await getViewLast7days();
      res.status(200).json(viewDay);
      break;
    case "month":
      const viewMonth = await getViewLast30days();
      res.status(200).json(viewMonth);
      break;
    case "year":
      const viewYear = await getPostViewsLastYear();
      res.status(200).json(viewYear);
      break;
    default:
      const viewDay2 = await getViewLast7days();
      res.status(200).json(viewDay2);
  }
};
